...
class Diary:
    ...
    def __init__(self, root, canvas, year, month, day, Id):
        wd = os.getcwd()
        ...
        self.go_back_btn = Button(canvas, text="<", command=lambda: self.main_page(root, canvas, Id, year, month))
        ...
        try:
            with open(wd + "/Py_Korea_Project/user_data.json", "r") as file:
                data = json.load(file) 
                content = data[Id][self.date]
        except KeyError:
            pass
        else:
            self.txt.insert(END, content)

        save_btn = Button(canvas, text="Save", command=lambda: self.save_diary(root, canvas, Id, self.date, year, month))
    ...
    def main_page(self, root, canvas, Id, year, month):
        main_page = create_diary.MainPage(root, canvas, Id, year, month)

    def save_diary(self, root, canvas, Id, date, year, month):
        wd = os.getcwd()
